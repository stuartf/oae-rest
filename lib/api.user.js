 ...
 * @param  {RestContext}    [additionalOptions.tenantAlias]      The alias of the tenant to create the user on. If not provided the user will be created on the current tenant.
...
var createUser = module.exports.createUser = function(restCtx, username, password, displayName, additionalOptions, callback) {
    ...
    if (additionalOptions.tenantAlias) {
        RestUtil.RestRequest(restCtx, '/api/user/' + additionalOptions.tenantAlias + '/create', 'POST', params, callback);
    } else {
